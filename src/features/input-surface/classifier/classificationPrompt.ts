...
- **chat**: The user wants to have a free-form conversation with the AI assistant. Look for: questions, discussion prompts, exploratory dialogue, requests for explanation, brainstorming, "what do you think", general inquiries.
...
- Conversational question or discussion prompt = chat
...
Where category is one of: sql, python, literature, chat, note, data_ingest`;
...
export const FEW_SHOT_EXAMPLES = [
  ...
  // Literature examples
  { input: "Recent papers on CRISPR-Cas9 in methanotrophs", output: { type: 'literature', confidence: 0.95, alternatives: [{ type: 'chat', confidence: 0.03 }] } },
  { input: "What studies have examined soil microbiome response to drought?", output: { type: 'literature', confidence: 0.92, alternatives: [{ type: 'chat', confidence: 0.05 }] } },
  { input: "Find publications about nitrogen cycling in wetlands", output: { type: 'literature', confidence: 0.94, alternatives: [] } },
  { input: "Has anyone published on viral diversity in deep sea vents?", output: { type: 'literature', confidence: 0.90, alternatives: [{ type: 'chat', confidence: 0.06 }] } },
  { input: "Search for reviews on horizontal gene transfer in archaea", output: { type: 'literature', confidence: 0.93, alternatives: [] } },

  // Chat examples
  { input: "What do you think about using metagenomics for soil analysis?", output: { type: 'chat', confidence: 0.92, alternatives: [{ type: 'literature', confidence: 0.05 }] } },
  { input: "Can you explain the difference between 16S and shotgun sequencing?", output: { type: 'chat', confidence: 0.90, alternatives: [{ type: 'note', confidence: 0.07 }] } },
  { input: "Help me brainstorm approaches for analyzing this microbiome dataset", output: { type: 'chat', confidence: 0.88, alternatives: [{ type: 'note', confidence: 0.08 }] } },
  { input: "What are the pros and cons of rarefaction in diversity analysis?", output: { type: 'chat', confidence: 0.85, alternatives: [{ type: 'literature', confidence: 0.10 }] } },
  { input: "How should I interpret a Shannon diversity index of 3.5?", output: { type: 'chat', confidence: 0.87, alternatives: [{ type: 'literature', confidence: 0.08 }] } },
  ...
  { input: "The extraction protocol seems to work better at room temperature", output: { type: 'note', confidence: 0.78, alternatives: [{ type: 'chat', confidence: 0.18 }] } },
